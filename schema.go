...
// from github.com/graphql-go/graphql:
const introQuery = ` 
{
    __schema {
      queryType { name }
...
func printType(t OutType) {
	stype := "type"
	if t.IsEnum {
		stype = "enum"
	}
	comment := getComment(t)
	fmt.Println("\n"+escMagenta+stype+escNoColor, t.Name+" {", comment)
	for _, f := range t.Children {
		if f.Type == "enum" {
			...
			fmt.Println(col+"  "+f.Name, getComment(f))
			continue
		}
		str := getFieldStr(f)
		str += getComment(f)
	...
	fmt.Println(escNoColor + "}")
}

func printTypes(prefix string, types []OutType) {
	for _, t := range types {
		printType(t)
	}
	fmt.Println("")
...
func PrintSchemaFromUrl(surl string, inDataStruct, debug bool) (err error) {
	var out struct {
	...
	return PrintSchemaFromJson(body, inDataStruct)
}

func PrintSchemaFromJson(jsonData []byte, inDataStruct bool) (err error) {
	var result Result
	...
	for _, t := range result.Schema.Types {
		if !strings.HasPrefix(t.Name, "__") && t.Name != "String" && t.Name != "Boolean" && t.Name != "Float" {
